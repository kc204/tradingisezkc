...
const FirmSentimentCard: React.FC<FirmSentimentCardProps> = ({ firm, data }) => {
  ...
  return (
    <Card className="h-full bg-black/20 backdrop-blur-sm border border-white/10 text-white shadow-lg">
      <CardHeader>
        <div className="flex items-center gap-4 mb-2">
            <Image src={firm.logoUrl} alt={`${firm.name} logo`} width={48} height={48} className="rounded-lg bg-gray-800 p-1 object-contain border border-white/10" data-ai-hint="logo" />
            <div>
            <CardTitle className="text-white">{firm.name}</CardTitle>
            <CardDescription className="text-gray-400">Most Recent Weekly Analysis</CardDescription>
            </div>
        </div>
      </CardHeader>
      <CardContent className="space-y-5">
        <div>
          <h3 className="font-semibold text-white/90 mb-2 flex items-center"><Globe className="w-4 h-4 mr-2 text-primary"/> Web Summary</h3>
          <p className="text-gray-300 text-sm leading-relaxed">{data.summary}</p>
        </div>
        <div>
          <h4 className="font-semibold text-green-400 mb-2 flex items-center"><TrendingUp className="w-4 h-4 mr-2"/> Common Positives</h4>
